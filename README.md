...
![Демонстрационный урок в Omni Workspace](assets/demo.png)
...
1. Введите логин и пароль академии в форме Workspace и нажмите **«Войти в Omni»**.
2. Локальный адаптер запустит Edge в фоне и отправит запрос штатному API Omni.
3. Если Omni требует проверку браузера, нажмите **«Открыть официальный вход»**, пройдите проверку в Edge и повторите вход в форме либо завершите его в официальном окне.
4. Если окно Edge открыто, не закрывайте его, пока работаете.

Пароль из формы проходит через локальный сервер на `127.0.0.1` к официальному HTTPS API Omni; приложение не записывает его на диск, в localStorage или логи. Поле пароля очищается после отправки. Альтернатива — вводить пароль только в официальном окне. Приложение использует отдельный профиль Edge в `.omni-browser/`; вход из вашей обычной Opera или Edge туда не переносится. Сохранённая сессия может использоваться, пока Omni её принимает. Полный вход с реальными учётными данными через новую форму ещё требует пользовательской проверки.
...
- Cookies, CSRF и идентификатор сессии Omni не передаются во фронтенд Workspace. Пароль из формы обрабатывается только на время запроса входа и не сохраняется приложением.
...
Вход через форму запускает Edge без окна. Кнопка официального входа переключает собственную фоновую сессию в видимое окно для ручной проверки DDoS-Guard. Защита Omni может не принять фоновый режим; тогда используйте официальный вход. Уже открытое окно подключения не скрывается автоматически.
...
assets/             Публичный скриншот демонстрации
```
...
Локальные исследовательские заметки и материалы для агента хранятся в `.agent-notes/`, исключённой из Git и доступа через Vite. Не кладите их в `public/`: содержимое этой папки публикуется при сборке. Ранее закоммиченные заметки остаются в истории Git.
...
В Settings → Pages нужно выбрать источник **GitHub Actions**. Приватному репозиторию нужен тариф GitHub с поддержкой Pages для приватных репозиториев; workflow не меняет видимость репозитория.

`npm run build:pages` собирает демонстрационную бета-версию для пути `/omni-workspace/`. В ней нет формы реального входа: GitHub Pages не запускает Node.js/Playwright. Реальная интеграция остаётся доступна при локальном запуске. При переименовании репозитория обновите `base` в `vite.config.ts`. Настройка workflow сама по себе не означает, что сайт уже опубликован.
...
[История выпусков](CHANGELOG.md). Иконка создана генеративным инструментом; официальные логотипы академии и браузеров не использованы.
