...
export function LiveWorkspace({ onDemo }: { onDemo: () => void }) {
  ...
  return <div className="app-shell live-workspace">
    ...
    <main><header className="topbar"><div className="breadcrumb"><strong>Рабочее пространство преподавателя</strong></div><span className={`demo-badge ${snapshot && !authNeeded ? 'connected-badge' : ''}`}><span/>{snapshot ? 'Omni подключён' : 'Подключение к Omni'}</span></header><div className="page-content">
      ...
      {!snapshot && !switchingTeacher && <section className="panel connection-card"><span className="lesson-icon"><LogIn size={26}/></span><h2>{waiting ? 'Завершите вход в окне Omni' : 'Вход в Omni'}</h2><p>{waiting ? connectionState === 'challenge' ? 'Пройдите проверку браузера в открывшемся Edge. Затем можно войти здесь или в официальном окне.' : 'Можно завершить вход в официальном окне или использовать форму ниже.' : 'Введите данные аккаунта академии. Подключение работает через Edge в фоне; при проверке браузера потребуется официальное окно.'}</p><LoginForm busy={busy} onLogin={login}/><button className="button secondary" disabled={busy} onClick={() => void connect()}><ExternalLink size={17}/>Открыть официальный вход</button><div className="connection-help"><ShieldCheck size={16}/>Сессия остаётся на этом компьютере. Демо-данные не попадают в реальный аккаунт.</div><button className="text-button" onClick={onDemo}>Пока посмотреть демонстрацию<ArrowRight size={15}/></button></section>}
      {snapshot && <>
