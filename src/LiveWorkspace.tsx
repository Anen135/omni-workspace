...
import { ArrowRight, BookOpen, CalendarDays, CheckCheck, ClipboardCheck, Download, ExternalLink, FolderOpen, GraduationCap, History, LoaderCircle, LogIn, RefreshCw, ShieldCheck, Users, X, Zap, Calendar, ChevronDown, Search } from 'lucide-react';
...
export function LiveWorkspace({ onDemo }: { onDemo: () => void }) {
  ...
  function weekForDate(value: string) {
    const chosen = new Date(`${value}T12:00:00`); const now = new Date(`${today}T12:00:00`);
    const monday = (date: Date) => { const day = date.getDay() || 7; date.setDate(date.getDate() - day + 1); return date; };
  ...
  const studentTable = (items: RemoteRecord[]) => { const query = studentQuery.trim().toLocaleLowerCase('ru-RU'); const visible = items.filter(student => !query || label(student.fio_stud).toLocaleLowerCase('ru-RU').includes(query)); return <><div className="table-toolbar compact-toolbar"><label className="search"><Search size={14}/><input aria-label="Поиск ученика" value={studentQuery} onChange={event => setStudentQuery(event.target.value)} placeholder="Поиск ученика"/></label><span className="table-count">{visible.length} из {items.length}</span></div><div className="table-scroll"><table><thead><tr><th>УЧЕНИК</th><th>ПРИСУТСТВИЕ</th><th>КОНТРОЛЬНАЯ</th><th>РАБОТА НА УРОКЕ</th></tr></thead><tbody>{visible.map((student, index) => <tr key={label(student.id_stud) || index}><td><button className="student-name" disabled={busy || !student.id_stud} onClick={() => void openStudent(student)}><span className="avatar purple">{label(student.fio_stud).split(' ').slice(0, 2).map(part => part[0]).join('')}</span>{label(student.fio_stud) || 'Имя не получено'}</button></td><td>{({ '0': 'Отсутствует', '1': 'Присутствует', '2': 'Опоздал(а)' } as Record<string, string>)[label(student.was)] || 'Не отмечено'}</td><td>{label(student.mark2) || '—'}</td><td>{label(student.mark4) || '—'}</td></tr>)}</tbody></table></div></>; };
  ...
  return <div className="app-shell live-workspace">
    ...
    <main><header className="topbar"><div className="topbar-workspace"><span className="workspace-mark">o<span/></span><div><strong>Академия TOP</strong><small>{snapshot?.account.branch || 'Omni Workspace'}</small></div></div><div className="topbar-actions">{snapshot && <button className="button secondary compact" disabled={busy} onClick={() => void refresh()}><RefreshCw size={14} className={busy ? 'spin' : ''}/>Обновить</button>}{snapshot && <label className="act-as"><Users size={14}/><span>Работаю от имени</span><select aria-label="Работаю от имени" value={snapshot.account.id} disabled={busy || !teachers.length || (isDesktop() || isExtension())} onChange={event => void switchTeacher(event.target.value)}><option value={snapshot.account.id}>{snapshot.account.name}</option>{teachers.map(teacher => <option key={label(teacher.id_teach)} value={label(teacher.id_teach)}>{label(teacher.fio_teach)}</option>)}</select><ChevronDown size={13}/></label>}<span className={`demo-badge ${snapshot && !authNeeded ? 'connected-badge' : ''}`}><span/>{snapshot ? 'Подключено' : 'Ожидает входа'}</span></div></header><div className="page-content">
      <div className="live-mobile-nav"><select aria-label="Раздел" value={tab} onChange={event => setTab(event.target.value as Tab)}><option value="lesson">Мой урок</option><option value="schedule">Расписание</option><option value="groups">Ученики</option><option value="homework">Домашние задания</option><option value="materials">Материалы</option></select><button className="text-button" onClick={onDemo}>Демо</button></div>
       <div className="page-heading live-heading compact-heading"><div><div className="eyebrow">{tab === 'lesson' ? 'ТЕКУЩАЯ РАБОТА' : tab === 'schedule' ? 'НЕДЕЛЬНЫЙ ОБЗОР' : tab === 'groups' ? 'УЧЕБНЫЕ ГРУППЫ' : tab === 'homework' ? 'ПРОВЕРКА РАБОТ' : 'БИБЛИОТЕКА'}</div><h1>{({ lesson: 'Мой урок', schedule: 'Расписание', groups: 'Ученики', homework: 'Домашние задания', materials: 'Материалы' } as Record<Tab, string>)[tab]}</h1></div>{tab === 'schedule' && <div className="schedule-tools"><button className="button secondary compact" disabled={busy} onClick={() => void refresh(week - 1)}>←</button><span>{label(record(schedule.start_end).monday)} — {label(record(schedule.start_end).sunday)}</span><button className="button secondary compact" disabled={busy} onClick={() => void refresh(week + 1)}>→</button><button className="icon" aria-label="Выбрать дату" onClick={() => setCalendarOpen(value => !value)}><Calendar size={16}/></button>{calendarOpen && <div className="date-popover"><label>Перейти к дате<input type="date" defaultValue={today} onChange={event => { setCalendarOpen(false); void refresh(weekForDate(event.target.value)); }}/></label><button className="text-button" onClick={() => { setCalendarOpen(false); void refresh(0); }}>Сегодня</button></div>}</div>}</div>
      ...
      {snapshot && <>
        ...
        {tab === 'schedule' && <section className="panel"><div className="panel-heading"><div><h2>Расписание</h2><p>{label(record(schedule.start_end).monday)} — {label(record(schedule.start_end).sunday)}</p></div><div className="live-week"><button className="button secondary" aria-label="Предыдущая неделя" disabled={busy || week <= -52} onClick={() => void refresh(week - 1)}>←</button><button className="button secondary" disabled={busy} onClick={() => void refresh(0)}>Эта неделя</button><button className="button secondary" aria-label="Следующая неделя" disabled={busy || week >= 52} onClick={() => void refresh(week + 1)}>→</button></div></div>{snapshot.schedule.error ? <SectionView section={snapshot.schedule}/> : <ScheduleView data={schedule} today={today}/>}</section>}
         {tab === 'groups' && <section className="panel"><div className="panel-heading"><div><h2>{groupName || 'Мои группы'}</h2><p>Просмотр учеников и доступных оценок</p></div></div>{snapshot.groups.error ? <SectionView section={snapshot.groups}/> : groups.length ? <><div className="live-group-buttons">{groups.map((group, index) => <button className={`button secondary ${groupName === label(group.name_tgroups) ? 'active-chip' : ''}`} disabled={busy || !group.id_tgroups} key={label(group.id_tgroups) || index} onClick={() => void openGroup(group)}>{label(group.name_tgroups) || 'Группа'}</button>)}</div>{groupStudents && (rows(groupStudents.data).length ? studentTable(rows(groupStudents.data)) : <Empty title="В группе нет доступных учеников" text="Omni вернул пустой список."/>)}</> : <Empty title="Omni пока не возвращает группы" text="Подключение работает. Список групп пуст в самом аккаунте академии."/>}</section>}
    ...
    {detail && <dialog ref={dialog} onCancel={() => setDetail(null)} aria-label={detail.title}><div className="modal-header"><h2>{detail.title}</h2><button className="icon" aria-label="Закрыть" onClick={() => setDetail(null)}><X size={20}/></button></div><div className="modal-body">{detail.sections.map((section, index) => <SectionView key={index} section={section}/>)}</div></dialog>}
     {notice && <div className="toast" role="status"><CheckCheck size={17}/>{notice}<button className="icon" aria-label="Закрыть уведомление" onClick={() => setNotice('')}><X size={16}/></button></div>}<button className={`quick-handle ${quickOpen ? 'open' : ''}`} aria-label="Быстрые действия" onClick={() => setQuickOpen(value => !value)}><Zap size={16}/></button>{quickOpen && <aside className="quick-drawer"><div><strong>Быстрые действия</strong><button className="icon" aria-label="Закрыть" onClick={() => setQuickOpen(false)}><X size={15}/></button></div>{snapshot && <button onClick={exportSnapshot}><Download size={15}/>Экспорт снимка</button>}<button onClick={() => void connect()}><ExternalLink size={15}/>Открыть Omni</button>{tab === 'schedule' && <button onClick={() => window.print()}><CalendarDays size={15}/>Печать расписания</button>}</aside>}
  </div>;
}
